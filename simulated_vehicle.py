...
def create_simulated_vehicle(
    namespace: str,
    start_pose_lat_lon: Tuple[float, float, float],
    goal_position_lat_lon: Tuple[float, float],
    vehicle_id: int,
    v2x_id: int,
    ...
    start_pose_utm_x, start_pose_utm_y, start_pose_utm_zone_number, start_pose_zone_letter = utm.from_latlon(start_pose_lat_lon[0], start_pose_lat_lon[1])
    goal_position_utm_x, goal_position_utm_y, goal_position_utm_zone_number, goal_position_zone_letter = utm.from_latlon(goal_position_lat_lon[0], goal_position_lat_lon[1])
    ...
    return [
        Node(
            ...
            namespace=namespace,
            parameters=[
                {"set_start_utm_position_x": float(start_pose_utm_x)},
                {"set_start_utm_position_y": float(start_pose_utm_y)},
                {"set_start_utm_zone_number": start_pose_utm_zone_number},
                {"set_start_utm_zone_letter": start_pose_zone_letter},
                {"set_start_psi": start_pose_lat_lon[2]},
        ...
        Node(
            ...
            parameters=[
                ...
                {"goal_position_x": float(goal_position_utm_x)},
                {"goal_position_y": float(goal_position_utm_y)},
